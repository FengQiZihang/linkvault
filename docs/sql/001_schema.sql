...
CREATE TABLE `lv_user` (
  ...
  `status` VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '用户状态：ACTIVE/DELETED',
...
CREATE TABLE `lv_link` (
  ...
  `original_url` VARCHAR(2048) NOT NULL COMMENT '用户粘贴的原始URL',
...
CREATE TABLE `lv_bookmark` (
  ...
  `status` VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '收藏状态：ACTIVE/DELETED',
  ...
  KEY `idx_bookmark_user_status` (`user_id`, `status`, `saved_at`),
  KEY `idx_bookmark_link` (`link_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户收藏关系与私有备注';
...
CREATE TABLE `lv_tag` (
  ...
  `status` VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '标签状态：ACTIVE/DELETED',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_tag_user_name` (`user_id`, `name`),
  KEY `idx_tag_user_pinned` (`user_id`, `is_pinned`, `created_at`),
  KEY `idx_tag_user_status` (`user_id`, `status`)
